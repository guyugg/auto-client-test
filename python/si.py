...
【SI 控端自動化測試腳本 (骰寶 / SI)】
===============================================================================
說明: 本腳本用於自動化發送 SI (骰寶) 桌台控端指令（下注、搖骰開蓋、確認結算）。
"""
...
# =============================================================================
# 伺服器與連線設定
WEBSOCKET_URL = "ws://10.80.41.31:8041/api/ws?account=qasiboss&password=aaaa1234"
...
# 測試結果模式 (0: 圍 [1,1,1], 1: 單 [1,1,5], 2: 雙 [4,5,5])
# 說明: 連8單、連8双、連5單、1双、連6單、連3双、連3單、連9双、1單、連8双
TEST_RESULTS = "00000000010000000"
...
def get_points(result_char):
    """根據測試字元轉換對應的 3 個骰子點數 (0:圍, 1:單, 2:雙)"""
    if result_char == "0":
        return [1, 1, 1]
    elif result_char == "1":
        return [1, 1, 5]
...
def bet():
    """發送開啟下注指令"""
    global is_running, points, result_idx
    ...
    is_running = True
    points = get_points(TEST_RESULTS[result_idx])
    log_info(f"第 {result_idx} 筆 骰點結果: {points}")
    result_idx += 1
...
def on_message(ws, message):
    global round_id, points, DEALER_ID, DEALER_NAME, DEALER_ACCOUNT
    ...
    try:
        ...
        if action == "LoginSuccess":
            ...
            is_dicecup_ready = data.get("isDicecupReady")
            ...
            # 進入 running 階段 $\rightarrow$ 發送 Confirm 結算
                ...
                confirm(round_id, points)

            # 結算完成且骰盅就緒 $\rightarrow$ 開啟下一局下注
            elif status == "confirm" and is_dicecup_ready is True:
                bet()
